...
import type { InstantPayEventEmitter, PayButtonParams, PaymentRequest, RequestPaymentResult, InstantPayProvider } from '@tonkeeper/instantpay-protocol';
...
export class TonConnectAdapter implements InstantPayProvider {
  ...
  // Track DOM mount root (read/write)
  // Remove mount container tracking to avoid unused var errors; can be re-added when needed
  constructor(
  ...
  setPayButton(params: PayButtonParams): void {
    ...
    if (this.currentRequest && this.currentRequest.invoiceId !== req.invoiceId) {
      (this.events as any).emit({ type: 'cancelled', request: this.currentRequest, reason: 'replaced' });
    }
  ...
  hidePayButton(): void {
    ...
    (this.events as any).emit({ type: 'cancelled', request: req, reason: 'app' });
  ...
  async requestPayment(request: PaymentRequest): Promise<RequestPaymentResult> {
    ...
    try {
      ...
      if (request.asset.type === 'ton') {
        ...
        if (!userAddress) {
          ...
          if (!newUserAddress) {
            ...
            return { status: 'cancelled', reason: 'wallet' };
          }
        ...
        return { status: 'cancelled', reason: 'unsupported_env' };
      }
      ...
      if (this.abortedByApp) {
        this.isProcessing = false;
        this.currentRequest = null;
        return { status: 'cancelled', reason: 'app' };
      }
      this.isProcessing = false;
